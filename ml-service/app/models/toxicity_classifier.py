"""
SafeChat — Toxicity Classifier (Fine-tuned MuRIL)
...
class ToxicityClassifier:
    ...
    def load(self) -> None:
        """Load fine-tuned MuRIL model into memory."""
        ...
        try:
            ...
            self._loaded = True
            logger.success(f"MuRIL toxicity model loaded from {self._model_name}")
    ...
    def _predict_sync(self, text: str, context: Optional[List[str]] = None) -> Dict:
        ...
        Context-aware tokenization:
          ...
          token_type_ids so MuRIL can attend across context and message.
        """
        if not self._loaded:
